...
export const getSyllabusxTeam = async () => {
    const content = await contentful.getEntries({ content_type: 'syllabusxTeam'});
    const items = content.items;
    const sortedItems = items.sort((a, b) => {
        // @ts-expect-error
...
export const getTAndC = async () => {
    const content = await contentful.getEntries({ content_type: 'termsConditions'});
    const item = content.items[0];
    return item;
}

export const getPolicy = async () => {
    const content = await contentful.getEntries({ content_type: 'privacyPolicy'});
    const item = content.items[0];
    return item;
}