    ...
    return (
        <div>
            ...
            <div className="syll-practicals">
                {practical.map(d => (
                        <a href={d.webViewLink} target="_blank" rel="noopener noreferrer" className={`practical-title ${d.description ? d.description.includes('new') ? 'shine' : '' : '' }`} key={d.id} >{d.name.slice(0, -4)}</a>
                ))}
            </div>
